package com.hdpsolutions.docbaotest.database;
...
import com.hdpsolutions.docbaotest.dto.CategoryDto;
...
import com.hdpsolutions.docbaotest.dto.VariableDto;
import com.hdpsolutions.docbaotest.utils.Constant;
import com.hdpsolutions.docbaotest.database.TableUtils.*;
import com.hdpsolutions.docbaotest.database.SQLUtils.*;
import com.hdpsolutions.docbaotest.database.DatabaseHandler.*;
...
public class DatabaseHandler extends SQLiteOpenHelper {
    ...
    public int insertPaper(PaperDto paperDto) {
        ...
        try {
            ...
            values.put(TableUtils.PAPER.COLUMN_NAME, paperDto.getName());
            values.put(TableUtils.PAPER.COLUMN_ICON, paperDto.getIcon());
            values.put(TableUtils.PAPER.COLUMN_CHOOSE, paperDto.getChoose());
            values.put(TableUtils.PAPER.COLUMN_ACTIVE, paperDto.getActive());
            values.put(TableUtils.PAPER.COLUMN_DATE_FORMAT, paperDto.getDateFormat());
            _id = (int) db.insert(TableUtils.PAPER.TABLE_NAME, null, values);
            db.close();
        } catch (Exception ex) {
    ...
    public int countPaper() {
        ...
        try {
            ...
            String sql = "select count(*) as cpaper from " + TableUtils.PAPER.TABLE_NAME;
            Log.i(LOG, sql);
    ...
    public void updatePatientsActive(String[] papers) {
        try {
            SQLiteDatabase db = this.getWritableDatabase();
            String sqlWhere = "";
            db.execSQL("UPDATE " + TableUtils.PAPER.TABLE_NAME + " SET Active = 0");
            for (int i = 0; i < papers.length; i++) {
                if (sqlWhere == "")
                    sqlWhere += TableUtils.PAPER.COLUMN_NAME + " = '" + papers[i] + "'";
                else
                    sqlWhere += (" or " + TableUtils.PAPER.COLUMN_NAME + " = '" + papers[i] + "'");
            }
            String sql = "UPDATE " + TableUtils.PAPER.TABLE_NAME + " SET Active = 1 WHERE " + sqlWhere;
            db.execSQL(sql);
            db.execSQL("UPDATE " + TableUtils.PAPER.TABLE_NAME + " SET Choose = 0");
            db.execSQL("UPDATE " + TableUtils.PAPER.TABLE_NAME + " SET Choose = 1 WHERE Name = '" + Constant.DEFAULT_PAPERS[0] + "'");
            db.close();
        } catch (Exception ex) {
            Log.i(LOG, ex.toString());
        }
    ...
    public ArrayList<PaperDto> getPapersActive() {
        ...
        try {
            SQLiteDatabase db = this.getReadableDatabase();
            String sql = "select * from " + TableUtils.PAPER.TABLE_NAME + " where Active = 1 ORDER BY Name ASC";
            Log.i(LOG, sql);
            Cursor c = db.rawQuery(sql, null);
            if (c.moveToFirst()) {
                do {
                    PaperDto paperDto = new PaperDto();
                    paperDto.setId((int) c.getLong(c.getColumnIndexOrThrow(TableUtils.PAPER._ID)));
                    paperDto.setName(c.getString(c.getColumnIndex(TableUtils.PAPER.COLUMN_NAME)));
                    paperDto.setChoose(c.getInt(c.getColumnIndex(TableUtils.PAPER.COLUMN_CHOOSE)));
                    paperDto.setActive(c.getInt(c.getColumnIndex(TableUtils.PAPER.COLUMN_ACTIVE)));
                    paperDto.setDateFormat(c.getString(c.getColumnIndex(TableUtils.PAPER.COLUMN_DATE_FORMAT)));
                    paperDto.setIcon(c.getInt(c.getColumnIndex(TableUtils.PAPER.COLUMN_ICON)));
